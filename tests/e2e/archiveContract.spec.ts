...
test("every promoted archive contract completes through the job engine and validator", async ({ page }) => {
  ...
  for (const contract of VERIFIED_ARCHIVE_RECIPE_CONTRACTS) {
    const outputs = await page.evaluate((recipeId) => window.__omniArchiveHarness.runArchiveRecipe(recipeId), contract.recipeId);
