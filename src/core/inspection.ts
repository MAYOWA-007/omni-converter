...
function detectSignature(header: Uint8Array): ExactFormatId {
  ...
  if (matches(header, [0x77, 0x4f, 0x46, 0x46])) return "woff";
  if (matches(header, [0x77, 0x4f, 0x46, 0x32])) return "woff2";
  if (matches(header, [0x4f, 0x54, 0x54, 0x4f])) return "otf";
  if (matches(header, [0x00, 0x01, 0x00, 0x00])) return "ttf";
