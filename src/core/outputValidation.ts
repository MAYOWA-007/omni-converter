...
type ExpectedFormat =
  ...
  | "zip" | "docx" | "xlsx" | "pptx"
  | "mp4" | "mov" | "m4a" | "avif" | "webm" | "wav" | "mp3" | "ogg"
...
const EXTENSION_FORMATS: Readonly<Record<string, ExpectedFormat>> = {
  ...
  json: "json", jsonl: "jsonl", ndjson: "jsonl", webmanifest: "json", zip: "zip", docx: "docx", xlsx: "xlsx", pptx: "pptx", mp4: "mp4", mov: "mov", m4a: "m4a",
  avif: "avif", webm: "webm", wav: "wav", mp3: "mp3", ogg: "ogg", gz: "gzip", gzip: "gzip", tar: "tar", rar: "rar", "7z": "7z",
...
function detectSignature(header: Uint8Array): SignatureDetection {
  ...
  if (matches(header, [0x1a, 0x45, 0xdf, 0xa3])) return detected("webm");
  if (matches(header, asciiBytes("RIFF")) && matches(header, asciiBytes("WAVE"), 8)) return detected("wav");
  if (matches(header, asciiBytes("ID3")) || (header[0] === 0xff && (header[1] & 0xe0) === 0xe0)) return detected("mp3");
...
const ISO_FAMILY_BRANDS: Readonly<Record<"mp4" | "mov" | "m4a" | "avif", ReadonlySet<string>>> = {
...
function isIsoBaseMediaFormat(format: string): format is "mp4" | "mov" | "m4a" | "avif" {
  return format === "mp4" || format === "mov" || format === "m4a" || format === "avif";
}

async function inspectIsoBaseMedia(blob: Blob, expectedFormat: "mp4" | "mov" | "m4a" | "avif") {
  ...
  if (!allBrands.size || ![...ISO_FAMILY_BRANDS[expectedFormat]].some((brand) => allBrands.has(brand)) || detectedFormat !== expectedFormat) {
    return { detectedFormat, error: `ISO base media brands do not match the expected ${expectedFormat.toUpperCase()} family.` };
  }
  ...
  return { detectedFormat, error: undefined as string | undefined };
}

function detectIsoFamily(brands: ReadonlySet<string>): "mp4" | "mov" | "m4a" | "avif" | "unknown" {
...
function invalidIso(expectedFormat: "mp4" | "mov" | "m4a" | "avif", error: string) {
...
function validWebm(header: Uint8Array, size: number) {
  ...
  return 4 + length + value <= size && includesAscii(header.slice(4 + length, Math.min(header.length, 4 + length + value)), "webm");
}
