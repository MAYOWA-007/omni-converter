...
function ChooseScreen({ catalog, inspection, recipes, preflights, onSelect, onBack }: { catalog: ConversionCatalog; inspection: FileInspection; recipes: ConversionRecipe[]; preflights: Map<string, PreflightResult>; onSelect: (recipe: ConversionRecipe) => void; onBack: () => void }) {
  ...
  return (
    <section className="screen choose-screen">
      <button className="back-button" type="button" onClick={onBack}><ChevronLeft size={16} />New file</button>
      <FileHeader inspection={inspection} />
      <div className="stage-copy">
        <span>{countLabel}</span>
        <h1 ref={headingRef} tabIndex={-1}>What should it become?</h1>
      </div>
        ...
        <div className="conversion-filters" aria-label="Conversion filters">
          ...
          <label>
            ...
            <select value={output} onChange={(event) => setOutput(event.target.value)}>
              ...
              {outputs.map((item) => <option value={item} key={item}>{item}</option>)}
            </select>
          </label>
        </div>
      </div>
...
function controlLabel(control: string) { return ({ archiveSelection: "Files", outputFormat: "Output", timeline: "Timeline", trim: "Trim", crop: "Crop", rotation: "Rotation", aspectRatio: "Aspect ratio", resolution: "Resolution", frameRate: "Frame rate", frameInterval: "Frame interval", chapterInterval: "Chapters", audioGain: "Volume", audioFade: "Fades", sampleRate: "Sample rate", audioChannels: "Channels", bitDepth: "Bit depth", waveform: "Waveform", typography: "Typography", captions: "Captions", color: "Color", compression: "Compression", dataTypes: "Value types", formulaSafety: "Formula safety", headerMode: "Headers", pageOrder: "Pages", pageLayout: "Layout", pageSize: "Page size", sheetSelection: "Sheets", slideSelection: "Slides", margins: "Margins", metadata: "Title metadata", watermark: "Watermark", batchNaming: "Names", bundle: "Bundle" }[control] ?? control); }
